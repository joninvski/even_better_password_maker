...
public class EntryActivity extends Activity implements View.OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		etURL.setSelection(etURL.getText().length()); // Puts the cursor at the
														// end of the string
		...
		spProfiles.setOnItemSelectedListener(new OnItemSelectedListener() {
			public void onItemSelected(AdapterView<?> parentView,
				...
				editor.putInt(getString(R.string.LastSelectedProfile),
						last_selected);
	...
	public void updateProfileSpinner() {
		...
		int last_selected = mPrefs.getInt(
				getString(R.string.LastSelectedProfile), 0);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.actionBtnCopy:
			...
			ClipData clip = ClipData.newPlainText("Copied Text",
					new String(generatedPassword.getData()));
			clipboard.setPrimaryClip(clip);

			Toast.makeText(getApplicationContext(),
					R.string.password_copy_to_clipboard, Toast.LENGTH_SHORT)
					.show();
			} catch (PasswordGenerationException e) {
				Log.e(Constants.LOG,
						"Error in generating the new password" + e.getMessage());
				Toast.makeText(getApplicationContext(),
						"Error generating password", Toast.LENGTH_SHORT)
						.show();
